...
var internals = {
    schema: {
        ...
        exclude: Joi.alternatives(Joi.string(), [Joi.array().items(Joi.string(), Joi.string())]).default(null)
    }
};
...
internals.connectionInfo = function (routes, options, connectionInfo) {
    ...
    for (var i = 0, il = routes.length; i < il; ++i) {
        ...
        if (route.settings.plugins.blipp && route.settings.plugins.blipp.exclude) {
            continue;
        }

        if (options.exclude) {
            if (options.exclude.indexOf(route.path) !== -1) {
                continue;
            }
        }
