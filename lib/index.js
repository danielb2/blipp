...
internals.connectionInfo = function (server, routes, options, connectionInfo) {
    ...
    for (let i = 0; i < routes.length; ++i) {
        ...
        let authScope = false;
        if (route.settings.auth && route.settings.auth.access) {
            const required = route.settings.auth.access[0].scope.required ?
                `required: ${route.settings.auth.access[0].scope.required.toString()} ` : '';
            const forbidden = route.settings.auth.access[0].scope.forbidden ?
                `forbidden: ${route.settings.auth.access[0].scope.forbidden.toString()} ` : '';
            const selection = route.settings.auth.access[0].scope.selection ?
                `selection: ${route.settings.auth.access[0].scope.selection.toString()}` : '';
